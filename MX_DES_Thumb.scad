...
//Parameters
// The lower the lower the keycap (lower versions are required for both Choc V2 and Gateron KS33)
// heightShift = 0;     // Pseudoku variant
// heightShift = -3;   // Zzeneg variant (see minY-minZ variant)
heightShift = -2.5;   // Dohn-joh variant
...
module keycap(keyID = 0, cutLen = 0, visualizeDish = false, rossSection = false, Dish = true, Stem = false, crossSection = true,Legends = false, homeDot = false, Stab = 0) {
  ...
  difference(){
    union(){
      difference(){
        skin([for (i=[-1:layers-1]) transform(translation(CapTranslation(i, keyID)) * rotation(CapRotation(i, keyID)), elliptical_rectangle_profile(CapTransform(i, keyID), b = CapRoundness(i,keyID),fn=fn))]); //outer shell

        //Cut inner shell
        if(Stem == true){
          translate([0,0,-.001])skin([for (i=[-1:layers-1]) transform(translation(InnerTranslation(i, keyID)) * rotation(CapRotation(i, keyID, true)), elliptical_rectangle_profile(InnerTransform(i, keyID),fn=fn))]);
        }
...
MXLen = 4.03/2+Tol; //vertical length (MX and KS33)

MXWidT = 1.10/2+Tol; //horizontal thickness (MX and KS33)
MXLenT = 1.23/2+Tol; //vertical thickness (MX and KS33)
